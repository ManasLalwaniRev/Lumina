...
// ... (keep all the helper functions: snakeToCamel, formatDateForDisplay, formatDateForInput)

const EditDataModal = ({ onClose, userId, userRole, username }) => {
  const [records, setRecords] = useState([]);
  const [selectedRecord, setSelectedRecord] = useState(null);
  const [initialPdfPath, setInitialPdfPath] = useState(''); // Store the initial PDF path
  const [formData, setFormData] = useState({
    primeKey: '', creditCard: '', contractShortName: '', vendorName: '', chargeDate: '',
  ...
  useEffect(() => {
    if (selectedRecord) {
      setFormData(prev => ({
        ...prev,
        submitter: username || selectedRecord.submitter,
      }));
    } else if (username) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleRecordSelect = (record) => {
    setSelectedRecord(record);
    setInitialPdfPath(record.pdfFilePath || ''); // Store the initial path
    setFormData({
      ...record,
      chargeDate: formatDateForInput(record.chargeDate),
      submittedDate: formatDateForInput(record.submittedDate),
  ...
  const handleSave = async (e) => {
    ...
    if (
      userRole === 'user' &&
      formData.pdfFilePath !== initialPdfPath &&
      !formData.notes
    ) {
      setErrorMessage('A note is required when changing the PDF file path.');
      setShowErrorMessage(true);
  ...
  return (
    <Modal title="Edit Existing Entry" onClose={onClose}>
      <div className="p-6 sm:p-8 bg-white rounded-xl shadow-lg max-h-[90vh] overflow-y-auto">
        ...
        {/* Just ensure the 'notes' textarea has its required attribute set based on the new logic if you wish */}
        <form className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
            {/* All other input fields remain the same */}
            <div className="col-span-full">
            <label htmlFor="notes" className="block text-sm font-medium text-gray-700 mb-1">
              NOTES {userRole === 'user' && formData.pdfFilePath !== initialPdfPath && <span className="text-red-500">*</span>}
            </label>
            <textarea
              id="notes"
              placeholder="Add any additional notes here..."
              ...
              value={formData.notes}
              onChange={handleInputChange}
              required={userRole === 'user' && formData.pdfFilePath !== initialPdfPath}
            ></textarea>
          </div>
